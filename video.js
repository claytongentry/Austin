...
function playVideo(reel) {
    ...
    switch (reel) {
        case 'directing':
            $.getJSON('http://www.vimeo.com/api/oembed.json?url=' + encodeURIComponent('http://vimeo.com/7100569') + '&width=970&callback=?', function(data){
                $('#video').html(data.html);
                $('#video').append(close_button);
            });
            break;
        case 'cinematography':
            $.getJSON('http://www.vimeo.com/api/oembed.json?url=' + encodeURIComponent('https://vimeo.com/110931915') + '&width=970&callback=?', function(data){
                $("#video").html(data.html);
                $("#video").append(close_button);
            });
            break;
        case 'motion_graphics':
            $.getJSON('http://www.vimeo.com/api/oembed.json?url=' + encodeURIComponent('//') + '&width=970&callback=?', function(data){
                $("#video").html(data.html);
                $("#video").append(close_button);
            });
            break;
